...
READY_PATH = ROOT / 'arabialawer_ready.json'
REPORT_PATH = ROOT / 'arabialawer_add_report.json'
...
def main():
    ...
    REPORT_PATH.write_text(json.dumps({
        'added': len(additions),
        'by_category': {category: sum(1 for item in additions if item['category'] == category) for category in sorted({item['category'] for item in additions})},
        'stats': stats,
    }, ensure_ascii=False, indent=2), encoding='utf-8')
    print(json.dumps({
        'added': len(additions),
        'total': stats['total'],
        'qadaa': stats['qadaa_count'],
        'nizam': stats['nizam_count'],
        'mohama': stats['mohama_count'],
    }, ensure_ascii=False))
